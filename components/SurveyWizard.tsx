...
function isAnswered(q: Question, vals: Values) {
  const v = vals[q.code];
  if (q.qtype === 'text' || q.qtype === 'longtext') return typeof v === 'string' && v.trim().length > 0;
  if (q.qtype === 'number') return v !== null && v !== undefined && v !== '';
  if (q.qtype === 'single') {
    if (!v) return false;
    const hasOther = q.options?.some(o => o.value === 'other');
    if (hasOther && v === 'other') return typeof vals[`${q.code}_other`] === 'string' && vals[`${q.code}_other`].trim().length > 0;
    return true;
  }
  if (q.qtype === 'multi') {
    if (!Array.isArray(v) || v.length === 0) return false;
    if (v.includes('other')) return typeof vals[`${q.code}_other`] === 'string' && vals[`${q.code}_other`].trim().length > 0;
    return true;
  }
  return false;
}
...
}: { q: Question; value: any; onChange: (v: any) => void; error?: string | null }) {
  return (
          ...
          <div className="grid gap-2">
            {q.options?.map((opt) => (
              <label key={opt.id} className="inline-flex items-center gap-2 text-base">
                <input
                  type="radio" name={q.code} className="size-4"
          ...
          <div className="grid gap-2">
            {q.options?.map((opt) => {
              ...
              return (
                <label key={opt.id} className="inline-flex items-center gap-2 text-base">
                  <input
                    type="checkbox" className="size-4" checked={checked}
                    onChange={(e) => {
            ...
            {q.options?.some(o => o.value === 'other') && Array.isArray(value) && value.includes('other') && (
              <input
                placeholder="Please specify"
                className="mt-2 w-full rounded-lg border border-gray-300 px-3 py-2 text-base focus:outline-none focus:ring-2 focus:ring-black"
                onChange={(e) => onChange({ _other: e.target.value })}
...
export default function SurveyWizard({ survey, questions }: { survey: Survey; questions: Question[] }) {
  const steps = useMemo(() => {
  ...
  const answered = useMemo(() => {
    let c = 0; for (const q of questions) if (isAnswered(q, values)) c++; return c;
  }, [questions, values]);
  const progressPct = Math.round((answered / totalQuestions) * 100);

  function validateStep(sIdx: number) {
  ...
  function change(code: string, v: any) {
  ...
  async function submit() {
    // final validation across all questions
    let allOk = true; const next: Errors = {};
    for (const q of questions) { const ok = isAnswered(q, values); next[q.code] = ok ? null : 'This question is required.'; if (!ok) allOk = false; }
    setErrors(next);
    if (!allOk) { const idx = steps.findIndex(st => st.qs.some(q => !isAnswered(q, values))); if (idx >= 0) setStep(idx); return; }
    ...
    try {
      ...
      for (const q of questions) {
        ...
        if (q.qtype === 'single') {
          ...
          answers.push({ question_id: q.id, option_id: opt?.id || null, value_text: v === 'other' ? (values[`${q.code}_other`] || '') : null, value_number: null });
        } else if (q.qtype === 'multi') {
          const arr: string[] = Array.isArray(v) ? v : [];
          for (const val of arr) {
            const opt = q.options?.find(o => o.value === val);
            answers.push({ question_id: q.id, option_id: opt?.id || null, value_text: val === 'other' ? (values[`${q.code}_other`] || '') : null, value_number: null });
          }
        } else if (q.qtype === 'number') {
          answers.push({ question_id: q.id, option_id: null, value_text: null, value_number: typeof v === 'number' ? v : Number(v) });
        } else { // text / longtext
          answers.push({ question_id: q.id, option_id: null, value_text: v || '', value_number: null });
        }
      }
      ...
      const res = await fetch('/api/submit', {
        ...
        body: JSON.stringify({ survey_id: survey.id, institution, answers, token }),
      });
    ...
    <div className="mx-auto max-w-6xl p-4 md:p-8">
      ...
      <div className="mt-6 grid gap-6 md:grid-cols-[260px,1fr]">
            ...
            {steps.map((s, i) => {
              ...
              return (
                <li key={s.section}>
                  <button
                    onClick={() => setStep(i)}
                    className={`w-full text-left rounded-lg px-3 py-2 text-sm transition ${current ? 'bg-black text-white' : 'hover:bg-gray-100'}`}
                  >
        ...
        <section>
          <div className="space-y-5">
            {steps[step].qs.map(q => (
              <Field key={q.id} q={q} value={values[q.code]} onChange={(v) => change(q.code, v)} error={errors[q.code]} />
            ))}
          </div>
      ...
      <div className="md:hidden sticky bottom-0 left-0 right-0 mt-8 border-t bg-white/80 backdrop-blur supports-[backdrop-filter]:bg-white/60">
        <div className="mx-auto max-w-6xl p-4 flex items-center justify-between gap-3">
          ...
          {step < steps.length - 1 ? (
            <button onClick={() => { if (validateStep(step)) setStep(s => Math.min(steps.length - 1, s + 1)); }} className="px-5 py-2 rounded-lg bg-black text-white">
              Next
            </button>
          ) : (
            <button onClick={submit} disabled={submitting} className="px-5 py-2 rounded-lg bg-black text-white">
              {submitting ? 'Submitting…' : 'Submit'}
            </button>
          )}
        </div>
